...
>
>- 孤儿进程，父进程在子进程之前退出了程序，子进程会认PID为1的进程做父进程，由PID为1的进程负责释放
>
...
![image-20200831160525742](./Readme.assets\image-20200831160525742.png)
...
​                    ![image-20200831170300600](./Readme.assets\image-20200831170300600.png)           
...
- **TCP套接字缓冲区**
 ...
 ![image-20200831170350585](./Readme.assets\image-20200831170350585.png)
...
​                          ![image-20200831171254363](Readme.assets\image-20200831171254363.png)     
...
~~~shell
# 预处理，预处理器读取main.c中#开头的代码，将头文件中的内容，插入到main.i中
gcc -E main.c -o main.i

# 编译，将main.i中的代码翻译为汇编语言输出到main.s中
gcc -S main.i -o main.s

# 汇编，将main.s中的汇编代码翻译为机器语言指令输出到main.o中
gcc -c main.s -o main.o

# 链接，合并库中的函数到最后的可执行程序中，main程序可以被加载到内存中，由系统执行
gcc main.o -o main
~~~
...
// 有雷的用法
#define sqr(x) x*x 
printf(“2+2 的平方：%d”, sqr(2+2));	// 展开前
printf(“2+2 的平方：%d”, 2+2 * 2+2);	// 展后后
...
~~~C
uint8_t b = 0b11111110; 
b = b & 0b11111101;	//  要将什么位,置0; 就将被与数的什么位,置0; 
//	结果 0b11111100;
...
b = b | 0b00001111;	//  要将什么位,置1; 就将被或数的什么位,置1;
//  结果 0b11111111;
...
>- **^ 按位 异或**
>- 一个数异或另外一个数，结果，相同为0，不同为1
>- 和1异或，将某几位翻转

~~~c
uint8_t b = 0b11110000; 
b = b ^ 0b00001111;	//  要将什么位置反转，什么位置异或 1；
//  结果 0b11111111;
...
void main()
{
    ...
    // 联合体所有成员公用一块内存
    printf("%p\n",&a);      // 输出 000000dbf9bffb0c         
    printf("%p\n",&a.a);    // 输出 000000dbf9bffb0c    
    printf("%p\n",&a.b);    // 输出 000000dbf9bffb0c
    printf("%p\n",&a.c);    // 输出 000000dbf9bffb0c
    a.a = 0;
    a.a+=10;
    a.b+=10;
    a.c+=10;
...
>- 初始化列表，其中参数有对象也可以使用
>
>![image-20200827193507981](./Readme.assets\image-20200827193507981.png)
...
- 友元就是允许对方访问该类的私有成员
  ...
  - 全局函数作为友元
    ...
    ![image-20200827194032643](./Readme.assets\image-20200827194032643.png)
...
>**免密码登录**
>
>1. 生成秘钥（默认名称id_rea）：ssh-keygen -t rsa 
>2. 找到 .ssh 目录的id_rsa.pub 内容追加到 服务器 .ssh 目录下的 authorized_keys 文件中
...
# 间隔参数
a[::1]  # 正数等于正序 [0, 1, 2, 3, 4, 5, 6]
a[::-1] # 负数等于逆序 [6, 5, 4, 3, 2, 1, 0]
a[::2]  # 大小等于间隔 [0, 2, 4, 6] 逢2丢弃
a[::3]  # 大小等于间隔 [0, 3, 6]    逢2、3丢弃
~~~
...
py从3.7版本开始默认字典保证插入顺序等于取出顺序，注意mpy不保证
...
match command:
    case "ping":
       pass
    case "shutdown" | "reboot": # 逻辑或，匹配多种情况
        pass
    case ("move", x, y): # 匹配元组、列表 --> ("move", 100, 250)
        print(f"移动指令：目标坐标 (X: {x}, Y: {y})")
    case ("sensor", "temperature", temp) if temp > 40:  # 匹配元组、列表 --> ("sensor", "temperature", 45.5)
        print(f"🚨 警报！温度过高：{temp}°C，启动紧急散热！")
    case ("sensor", "temperature", temp):  # 匹配元组、列表 --> ("sensor", "temperature", 25.5)
        print(f"温度正常：{temp}°C")
    case {"action": "login", "user": username}: 
        # 匹配字典 -->{ "action": "login", "user": "Alice", "ip": "192.168.1.1"}
        print(f"用户登录请求，用户名：{username}")
    case int() | float(): # 匹配类型 --> 3.14159
        print(f"收到纯数值配置：{command}")
    case n if isinstance(n, str) and len(n) > 10: #--> "a" * 11
        print(f"收到过长的文本指令，长度为 {len(n)}")
    case _:	# 匹配所有内容
        print(f"未知或不支持的指令类型：{command}")
~~~
...
# is      就是 	==
# is not  就是 	!=
# 不过 is 比较的是内存地址，== 比较的是值
# ==可能被重写，自定义类默认的实现也是比较地址
# is可能有性能优势
~~~
